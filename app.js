...
(function () {
  ...
  var PLAN_KEY = "cm_plan_v2"; // bumped: plan is now day -> shift -> EDs

  var DAY_NAMES = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  var MONTHS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

  // shift time windows (weekday vs weekend)
  ...
  function trunkColor(routes) {
    var first = String(routes || "").split(/[-\s]/)[0].toUpperCase();
    return MTA[first] || "#444";
  }
  ...
  function leanColor(cat) {
    ...
    if (/tossup/i.test(cat)) return "#6b7785";
    return "#6b7785";
  }
  ...
  var state = {
    ...
    geo: {},
    selectedId: null,
    sort: "opportunity", search: "",
    shadingMode: "opportunity",
    tab: "locations",
    weekStart: null, activeDay: null, activeShift: "AM",
    plan: {},                 // { iso: { AM:[ed...], PM:[ed...] } }
  };

  var map, legend, districtLayer, markerLayer;
  var overlay = { hoods: null, subway: null, polls: null, groc: null };
  var renderers = {};
  var edCentroid = {};        // elect_dist -> L.latLng
  ...
  function edLabel(ed) {
    ed = String(ed); var ad = ed.slice(0, -3), edn = ed.slice(-3);
    return "AD " + Number(ad) + " · ED " + Number(edn);
  }
  ...
  function recencyLabel(loc) {
    ...
    return { text: d <= 7 ? "this week" : (weeks <= 1 ? "1 wk ago" : weeks + " wks ago"),
      cls: d >= 35 ? "stale" : (d <= 14 ? "fresh" : ""), days: d };
  }

  function lerp(a, b, t) { return a.map(function (x, i) { return Math.round(x + (b[i] - x) * t); }); }
  function rgb(c) { return "rgb(" + c[0] + "," + c[1] + "," + c[2] + ")"; }
  var RAMPS = { opportunity: [[255, 245, 224], [203, 24, 29]], turnout: [[235, 244, 255], [8, 69, 148]], coverage: [[235, 250, 238], [0, 109, 44]] };
  function rampColor(mode, t) { var r = RAMPS[mode] || RAMPS.opportunity; return rgb(lerp(r[0], r[1], clamp01(Math.sqrt(t)))); }
  ...
  // ---- summary + (browse-only) locations list --------------------------
  ...
  function renderList() {
    ...
    ul.innerHTML = list.map(function (loc) {
      ...
      return '<li class="loc-row' + active + '" data-id="' + escapeHtml(loc.location_id) + '">' +
        '<div class="loc-main">' +
          '<div class="name"><span class="swatch" style="background:' + rampColor("coverage", num(loc.weighted_person_days) / maxWeighted) + '"></span>' +
          escapeHtml(firstAlias(loc)) + "</div>" +
          '<div class="meta">Last canvassed <span class="recency ' + rec.cls + '">' + rec.text + "</span> · " +
          num(loc.raw_person_days) + " person-days · " + num(loc.unique_people_count) + " volunteers</div>" +
        "</div></li>";
    }).join("");
  ...
  function popupHtml(loc) {
    ...
    return '<div class="popup-title">' + escapeHtml(firstAlias(loc)) + "</div>" +
      ...
      (people.length ? '<div class="popup-dates"><strong>Volunteers:</strong> ' + escapeHtml(people.slice(0, 12).join(", ")) +
  ...
  // ====================================================================
  //  DISTRICTS — one always-present, clickable layer
  // ====================================================================
  ...
  function activeSet() {
    var day = state.plan[state.activeDay];
    return new Set((day && day[state.activeShift]) || []);
  }
  function districtStyle(feature) {
    var mode = state.shadingMode, p = feature.properties, sel = activeSet().has(String(p.elect_dist));
    var st = { color: sel ? "#1f6feb" : "#8aa0b6", weight: sel ? 2.6 : 0.4, opacity: 1 };
    if (mode === "none") { st.fill = true; st.fillColor = "#ffffff"; st.fillOpacity = 0.02; return st; }
    if (mode === "mayor2025") {
      st.fillColor = MAYOR_COLORS[p.top_mayor_rank1_candidate] || "#cfd6dd";
      st.fillOpacity = p.top_mayor_rank1_candidate ? 0.3 + clamp01(num(p.top_mayor_rank1_share)) * 0.45 : 0.15;
    } else if (mode === "boreslasher") {
      st.fillColor = BL_COLORS[p.bl_winner] || "#b48ead";
      st.fillOpacity = num(p.bl_total) > 0 ? 0.32 + Math.min(Math.abs(num(p.bl_margin)), 0.4) / 0.4 * 0.45 : 0.12;
    } else {
      var maxV = mode === "turnout" ? maxBallots : mode === "coverage" ? maxCoverage : maxOpportunity;
      st.fillColor = rampColor(mode, maxV > 0 ? districtMetric(p, mode) / maxV : 0); st.fillOpacity = 0.6;
    }
    return st;
  }
  function districtTooltip(p) {
    var mode = state.shadingMode, head = "ED " + p.election_district + " · AD " + p.assembly_district;
    if (mode === "mayor2025") {
      var w = p.top_mayor_rank1_candidate;
      return head + "<br>" + (w ? "<strong>" + escapeHtml(MAYOR_SHORT[w] || w) + "</strong> led with " + pct(p.top_mayor_rank1_share) : "No mayoral data");
    }
    if (mode === "boreslasher") {
      if (!num(p.bl_total)) return head + "<br>No primary votes";
      return head + "<br><span style='color:" + BL_COLORS.Bores + "'>Bores</span> " + num(p.bl_bores) + " (" + pct(p.bl_bores_share) + ") · " +
        "<span style='color:#b8860b'>Lasher</span> " + num(p.bl_lasher) + " (" + pct(p.bl_lasher_share) + ")<br><strong>" + escapeHtml(p.bl_winner) + "</strong>";
    }
    var ballots = num(p.ballots_cast), cov = num(p.distance_adjusted_weighted_person_days) || num(p.weighted_person_days);
    ...
    if (mode === "none") return head;
    return head + "<br><strong>" + ballots + "</strong> Dem voters (2025)<br>Coverage so far: " + fmt(cov) + (cov < 1 ? " — barely canvassed" : "");
  }
  function buildDistrictLayer(g) {
    districtLayer = L.geoJSON(g, {
      ...
      onEachFeature: function (f, layer) {
        ...
        layer.on("click", function () { if (state.tab === "plan") toggleEd(ed); });
      },
    }).addTo(map);
  }
  function refreshDistricts() {
    ...
    districtLayer.eachLayer(function (l) { l.setTooltipContent(districtTooltip(l.feature.properties)); });
    updateLegend();
  }
  ...
  function toggleHoods(on) {
    ...
    ensureData("neighborhoods").then(function (g) {
      overlay.hoods = L.geoJSON(g, {
        pane: "pane-hoods", renderer: renderers.hoods,
        style: { fill: false, color: "#6a4c93", weight: 1.4, opacity: 0.85, dashArray: "4 3" },
        onEachFeature: function (f, l) { l.bindTooltip(escapeHtml(f.properties.name || "Neighborhood"), { permanent: true, direction: "center", className: "hood-label" }); },
      }).addTo(map);
    });
  }
    ...
    ensureData("subway").then(function (g) {
      overlay.subway = L.geoJSON(g, {
        pointToLayer: function (f, latlng) {
          var col = trunkColor(f.properties.routes);
          return L.circleMarker(latlng, { pane: "pane-subway", renderer: renderers.subway, radius: 4.5, color: "#fff", weight: 1.5, fillColor: col, fillOpacity: 1 });
        },
  ...
  function pinIcon(cls, color, glyph) {
    return L.divIcon({ className: "", html: '<div class="' + cls + '" style="background:' + color + '">' + glyph + "</div>", iconSize: [18, 18], iconAnchor: [9, 9] });
  }
  function togglePolls(on) {
    ...
    ensureData("polls").then(function (g) {
      overlay.polls = L.geoJSON(g, {
        pointToLayer: function (f, latlng) {
          return L.marker(latlng, { icon: pinIcon("poll-pin", leanColor(f.properties.category), "🗳"), pane: "pane-polls" });
        },
        onEachFeature: function (f, l) {
          var p = f.properties;
          l.bindTooltip("<strong>" + escapeHtml(p.name || "") + "</strong><br>" + escapeHtml(p.address || "") +
            (p.neighborhood ? "<br>" + escapeHtml(p.neighborhood) : "") +
            (p.category ? "<br>Lean: " + escapeHtml(p.category) : "") +
            (p.priority ? " · Priority " + escapeHtml(p.priority) : "") +
            (p.bodies_am_pm ? "<br>Bodies needed (AM/PM): " + escapeHtml(p.bodies_am_pm) : ""), { sticky: true });
        },
      }).addTo(map);
    });
  }
  ...
  function updateLegend() {
    ...
    if (mode === "mayor2025") {
      el.innerHTML = "<h4>2025 Mayor — who led each ED</h4>" + cat(MAYOR_COLORS["Zohran Kwame Mamdani"], "Mamdani") +
        cat(MAYOR_COLORS["Andrew M. Cuomo"], "Cuomo") + cat(MAYOR_COLORS["Brad Lander"], "Lander") +
        '<div class="sub">Stronger color = bigger win.</div>' + dotNote; return;
    }
    if (mode === "boreslasher") {
      el.innerHTML = "<h4>2026 Dem Primary — who won each ED</h4>" + cat(BL_COLORS.Bores, "Bores") + cat(BL_COLORS.Lasher, "Lasher") +
        cat(BL_COLORS.Tie, "Tie / other") + '<div class="sub">Stronger color = bigger margin.</div>' + dotNote; return;
    }
    ...
    var ramp = ""; for (var i = 0; i <= 8; i++) ramp += '<span style="background:' + rampColor(mode, i / 8) + '"></span>';
    el.innerHTML = "<h4>" + titles[mode] + "</h4><div class='ramp'>" + ramp + "</div>" +
  ...
  function shiftArr(iso, shift, make) {
    var d = state.plan[iso]; if (!d) { if (!make) return []; d = state.plan[iso] = { AM: [], PM: [] }; }
    if (!d[shift]) d[shift] = [];
    return d[shift];
  }
  function toggleEd(ed) {
    var arr = shiftArr(state.activeDay, state.activeShift, true), i = arr.indexOf(ed);
    if (i === -1) arr.push(ed); else arr.splice(i, 1);
    savePlan(); refreshDistricts(); renderPlan();
  }
  function removeEd(iso, shift, ed) {
    var arr = shiftArr(iso, shift, false), i = arr.indexOf(ed);
    if (i !== -1) arr.splice(i, 1); savePlan(); refreshDistricts(); renderPlan();
  }

  function renderShiftBanner() {
    var b = document.getElementById("shift-banner"); if (!b) return;
    var d = parseISO(state.activeDay);
    b.innerHTML = "Now adding to <strong>" + dowName(d) + " " + prettyDate(d) + " · " + state.activeShift +
      " (" + shiftTime(d, state.activeShift) + ")</strong> — click districts on the map.";
  }
  ...
  function renderPlan() {
    ...
    container.innerHTML = weekDays().map(function (d) {
      ...
      var shifts = ["AM", "PM"].map(function (sh) {
        var eds = shiftArr(iso, sh, false);
        var isActive = (iso === state.activeDay && sh === state.activeShift) ? " active" : "";
        var chips = eds.map(function (ed) {
          return '<li class="ed-chip"><span class="ed-go" data-ed="' + ed + '">' + escapeHtml(edShort(ed)) +
            '</span><button class="rm-btn" data-rm="' + ed + '" data-day="' + iso + '" data-shift="' + sh + '">✕</button></li>';
        }).join("");
        return '<div class="shift-row' + isActive + '" data-day="' + iso + '" data-shift="' + sh + '">' +
          '<div class="shift-head"><span class="sh-name">' + sh + '</span><span class="sh-time">' + shiftTime(d, sh) + '</span>' +
          '<span class="sh-count">' + (eds.length ? eds.length + " ED" + (eds.length > 1 ? "s" : "") : "select") + "</span></div>" +
          '<ul class="ed-chips">' + chips + "</ul></div>";
      }).join("");
    ...
    Array.prototype.forEach.call(container.querySelectorAll(".shift-row"), function (row) {
      row.querySelector(".shift-head").addEventListener("click", function () {
        state.activeDay = row.getAttribute("data-day"); state.activeShift = row.getAttribute("data-shift");
        refreshDistricts(); renderPlan();
      });
    });
    ...
    Array.prototype.forEach.call(container.querySelectorAll(".ed-go"), function (g) {
      g.addEventListener("click", function (e) {
        e.stopPropagation(); var ed = g.getAttribute("data-ed"); var c = edCentroid[ed];
        if (c) map.setView(c, Math.max(map.getZoom(), 15));
      });
    });
  }
  ...
  function planAsText() {
    ...
    weekDays().forEach(function (d) {
      var iso = isoOf(d), any = ["AM", "PM"].some(function (sh) { return shiftArr(iso, sh, false).length; });
      if (!any) return;
      lines.push(dowName(d) + " " + prettyDate(d) + ":");
      ["AM", "PM"].forEach(function (sh) {
        var eds = shiftArr(iso, sh, false); if (!eds.length) return;
        lines.push("  " + sh + " (" + shiftTime(d, sh) + "): " + eds.map(edLabel).join(", "));
      });
      lines.push("");
    });
  ...
  function copyPlan() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(text).then(done, function () { window.prompt("Copy the plan:", text); });
  ...
  function printPlan() {
    var rows = weekDays().map(function (d) {
      ...
      var inner = ["AM", "PM"].map(function (sh) {
        var eds = shiftArr(iso, sh, false);
        return "<div class='sh'><strong>" + sh + " (" + shiftTime(d, sh) + ")</strong>: " + (eds.length ? eds.map(edLabel).join(", ") : "—") + "</div>";
      }).join("");
      return "<div class='d'><h3>" + dowName(d) + " " + prettyDate(d) + "</h3>" + inner + "</div>";
    }).join("");
    var html = "<html><head><title>Canvassing plan</title><style>body{font-family:-apple-system,Arial,sans-serif;margin:32px;color:#1f2b38}" +
      "h1{font-size:20px}h3{font-size:14px;margin:0 0 4px;border-bottom:1px solid #ccc;padding-bottom:3px}.d{margin-bottom:12px}" +
      ".sh{font-size:13px;margin:2px 0}</style></head><body><h1>Canvassing plan</h1><h2 style='font-size:14px;color:#555'>" +
  ...
  function switchTab(name) {
    ...
    if (name === "plan") { refreshDistricts(); renderPlan(); }
    setTimeout(function () { map.invalidateSize(); }, 50);
  }

  // ---- init ------------------------------------------------------------
  function makePanes() {
    var defs = [["pane-shading", 410], ["pane-hoods", 420], ["pane-subway", 440], ["pane-sites", 450], ["pane-polls", 620], ["pane-groc", 610]];
  ...
  function wireUi() {
    ...
    document.getElementById("plan-clear").addEventListener("click", function () {
      if (!confirm("Remove all districts from this week's plan?")) return;
      weekDays().forEach(function (d) { delete state.plan[isoOf(d)]; }); savePlan(); refreshDistricts(); renderPlan();
    });
  ...
  function init() {
    ...
    Promise.all([
      ...
      var districts = res[2];
      if (districts && res[3]) {
        // join Bores/Lasher votes into the districts layer by elect_dist
        var bl = {};
        res[3].features.forEach(function (f) { bl[String(f.properties.elect_dist)] = f.properties; });
